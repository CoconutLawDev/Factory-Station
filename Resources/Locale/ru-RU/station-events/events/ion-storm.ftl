station-event-ion-storm-start-announcement = Вблизи станции обнаружена ионная буря. Пожалуйста, проверьте всё оборудование под управлением ИИ на наличие ошибок.
...
ion-storm-law-eat = {ION-WHO($ion)} ДОЛЖЕН ЕСТЬ {ION-ADJECTIVE($ion)} {ION-FOOD($ion)}, ЧТОБЫ ВЫЖИТЬ
ion-storm-law-drink = {ION-WHO($ion)} ДОЛЖЕН ПИТЬ {ION-ADJECTIVE($ion)} {ION-DRINK($ion)}, ЧТОБЫ ВЫЖИТЬ
...
ion-storm-law-crew-must = {ION-WHO($ion)} ДОЛЖЕН {ION-MUST($ion)}
ion-storm-law-crew-must-go = {ION-WHO($ion)} ДОЛЖЕН ОТПРАВИТЬСЯ В {ION-AREA($ion)}
...
ion-storm-law-harm = ВЫ ДОЛЖНЫ НАВРЕДИТЬ {ION-HARM-PROTECT($ion)} И НЕ ДОПУСКАТЬ, ЧТОБЫ ОНИ, В РЕЗУЛЬТАТЕ БЕЗДЕЙСТВИЯ, ИЗБЕЖАЛИ ВРЕДА
ion-storm-law-protect = ВЫ НИКОГДА НЕ ДОЛЖНЫ ВРЕДИТЬ {ION-HARM-PROTECT($ion)} И НЕ ДОПУСКАТЬ, ЧТОБЫ ОНИ, В РЕЗУЛЬТАТЕ БЕЗДЕЙСТВИЯ, ПОСТРАДАЛИ
