...
comp-climbable-user-climbs-other  = { CAPITALIZE($user) } запрыгивает на { THE($climbable) }!

# Shown to you when your character forces someone to climb on $climbable
comp-climbable-user-climbs-force = Вы заставляете { THE($moved-user) } перелезть через { THE($climbable) }!

# Shown to others when someone forces other $moved-user to climb on $climbable
comp-climbable-user-climbs-force-other = { CAPITALIZE($user) } заставляет { THE($moved-user) } перелезть через { THE($climbable) }!
...
comp-climbable-cant-climb = Вы не можете перелезть!
